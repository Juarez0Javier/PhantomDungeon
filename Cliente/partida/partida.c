...
void calcularMovJugador (Entidad* jugador, Mapa* mapa, char direccion, tCola* movs) {
    ...
    if (dentroDelTerreno) {
        ...
        bool transitable =
        mapa -> terreno[nuevaY][nuevaX] == CAMINO ||
        mapa -> terreno[nuevaY][nuevaX] == PREMIO ||
        mapa -> terreno[nuevaY][nuevaX] == VIDA ||
        mapa -> terreno[nuevaY][nuevaX] == SALIDA;
...
void resolverMovimientos (Partida* partida, Mapa* mapa, tCola* movs, int* seccion) {
    ...
    if (mov.ent == &partida -> jugador) {
        ...
        switch (mapa -> terreno[mov.y][mov.x]) {
            case VIDA:
                partida -> vidasRestantes++;
                mapa -> terreno[mov.y][mov.x] = CAMINO;
                break;
